...
const getCategoryId = async (pool, categorySlug) => {
  try {
    const result = await pool
      .request()
      .input("slug", sql.NVarChar(100), categorySlug).query(`
        SELECT CategoryID 
        FROM Categories 
        WHERE Slug = @slug
      `);
...
const getCountryId = async (pool, countrySlug) => {
  try {
    const result = await pool
      .request()
      .input("slug", sql.NVarChar(100), countrySlug).query(`
        SELECT CountryID 
        FROM Countries 
        WHERE Slug = @slug
      `);
...
function validateAndTruncateUrl(url, maxLength = 450) {
  ...
  try {
    ...
    if (processedUrl.length > maxLength) {
      console.warn(
        `URL too long (${processedUrl.length} chars), truncating to ${maxLength} chars`
      );
...
async function insertMovie(movie, pool) {
  try {
    ...
    const result = await pool
      ...
      .input("TmdbVoteCount", sql.Int, movie.tmdb?.vote_count || 0).query(`
...
async function insertMovieRelations(movieId, movie, pool) {
  try {
    ...
    await pool.request().input("MovieID", sql.Int, movieId).query(`
      ...
      for (const country of movie.country) {
        ...
        if (countryId) {
          await pool
            .request()
            .input("MovieID", sql.Int, movieId)
            .input("CountryID", sql.Int, countryId).query(`
...
async function insertServerAndEpisodes(movieId, episodes, pool) {
  try {
    for (const episodeGroup of episodes) {
      // Insert server
      const serverResult = await pool
        .request()
        .input("Name", sql.NVarChar(100), episodeGroup.server_name)
        .input("Type", sql.VarChar(20), "embed").query(`
      ...
      await pool
        .request()
        .input("MovieID", sql.Int, movieId)
        .input("ServerID", sql.Int, serverId).query(`
      ...
      for (const episode of episodeGroup.server_data) {
        await pool
          ...
          .input("EmbedUrl", sql.NVarChar(500), episode.link_embed).query(`
