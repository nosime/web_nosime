...
const { 
  ...
  async function processPageRange(startPage, endPage, threadIndex) {
    ...
    try {
      ...
      console.log(`Thread ${threadIndex}: Processing pages ${startPage} to ${endPage}`);
        ...
        try {
          ...
          for (const item of pageData.items) {
            try {
              ...
              if (!movieData.movie) {
                console.log(`Thread ${threadIndex}: No movie data found for ${item.slug}`);
                continue;
              }
              ...
              try {
                ...
                console.log(`Thread ${threadIndex}: Successfully processed movie ${item.name} (${threadStats.processedMovies}/${globalProgress.processedMovies} total)`);
              } catch (dbError) {
                console.error(`Thread ${threadIndex}: Database error for movie ${item.slug}:`, dbError);
              ...
              console.error(`Thread ${threadIndex}: Error processing movie ${item.slug}:`, error);
              threadStats.failedMovies++;
              globalProgress.failedMovies++;
              continue;
            }
          ...
          console.error(`Thread ${threadIndex}: Error processing page ${page}:`, error);
          pool = await createConnection();
          continue;
        }
      }
  
      console.log(`Thread ${threadIndex}: Completed range ${startPage}-${endPage}`);
